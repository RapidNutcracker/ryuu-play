...
export class DarknessEnergySpecial extends EnergyCard {
  ...
  public fullName = 'Darkness Energy (Special) HS';

  public text = 'If the Pokémon Darkness Energy is attached to attacks, ' +
  ...
  public reduceEffect(store: StoreLike, state: State, effect: Effect): State {
    if (effect instanceof DealDamageEffect) {
      if (effect.source.cards.includes(this)) {
        ...
        if (checkPokemonType.cardTypes.includes(CardType.DARKNESS)) {
          effect.damage += 10;
        }
      }
    }
