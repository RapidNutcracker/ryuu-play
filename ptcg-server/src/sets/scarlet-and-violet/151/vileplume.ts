...
function* useFullBloomingEnergy(next: Function, store: StoreLike, state: State,
  self: Vileplume, effect: PlayPokemonEffect): IterableIterator<State> {
...
export class Vileplume extends PokemonCard {
  ...
  public powers: Power[] = [{
    name: 'Full Blooming Energy',
    powerType: PowerType.ABILITY,
    text:
      'When you play this Pokémon from your hand to evolve 1 of your Pokémon during your turn, ' +
      'you may look at the top 8 cards of your deck and attach any number of Basic Energy cards you ' +
  ...
  public reduceEffect(store: StoreLike, state: State, effect: Effect): State {
    if (effect instanceof PlayPokemonEffect && effect.pokemonCard === this) {
      const generator = useFullBloomingEnergy(() => generator.next(), store, state, this, effect);
